...
//   priceRaw?: number;
//   currencyCode: string;
//   stockLevel?: string; // fetched lazily on first interaction or pre-populated
// };
...
//   const stockLimitMap = useRef<Record<string, number>>({});
//   const [openBrands, setOpenBrands] = useState<Record<string, boolean>>({});
//   // React state map of stockLevel per item slug — drives UI (out of stock label, disabled +)
...
//   // Helper: update both the ref cache and React state for stock level
...
//       if (variantIdMap.current[item.id]) {
//         if (stockLimitMap.current[item.id] === undefined) {
//           const { data: pd } = await loadDetails({ variables: { slug: item.slug }, fetchPolicy: "network-only" });
//           const variant = pd?.product?.variants?.[0];
...
//   /** Always fetches fresh stock from backend so limit is never stale. */
//   const getStockLimit = useCallback(
//     async (item: GridItem): Promise<number> => {
//       const { data: pd } = await loadDetails({ variables: { slug: item.slug }, fetchPolicy: "network-only" });

//       // ── Raw backend response ──────────────────────────────────────────────
//       console.log("[stock] raw backend response:", JSON.stringify(pd, null, 2));
//       console.log("[stock] all variants from backend:", pd?.product?.variants?.map((v: any) => ({
//         id: v.id,
//         name: v.name,
...
//       console.log("[stock] selected variant:", { id: variant?.id, name: variant?.name, stockLevel });
...
//         // Refresh stock after add so UI reflects updated availability
...
//                     // Derive stock state from React state map (updated after each fetch)
//                     const itemStockLevel = stockLevelState[item.id]; // undefined until first fetch
...
//                     // Disable + when out of stock OR qty already at the X_UNITS_LEFT limit
...
//                           {!qty ? (
//                             // No quantity in cart yet — show Add to Cart or Out of Stock
//                             itemIsOutOfStock ? (
//                               <button
//                                 disabled
