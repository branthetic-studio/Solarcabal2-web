...
//         amount: chargeAmount,
//         email,
//         reference: activeOrderData?.activeOrder?.code !,
//         channels:
//           method === "bank"
//             ? ["bank_transfer"]
...
//         onSuccess: () => {
//           router.replace(
//             `/checkout/paystack-redirect?reference=${order!.id}&status=success&amount=${chargeAmount}`
//           );
//         },
//         onCancel: async () => {
...
//   const { items: localItems, updateQuantity: updateLocalQuantity, removeItem: removeLocalItem } = useLocalCart();
...
//         metadata: { order_id: order.id, order_code: order.code, payment_method: method },
//         onSuccess: () => {
...
//         metadata: { order_id: order.id, order_code: order.code, payment_method: method },
//         onSuccess: () => {
//           // clearCart is from LocalCartContext — clears localStorage + state
//           clearCart();
//           router.replace(`/checkout/paystack-redirect?reference=${order!.id}&status=success&amount=${chargeAmount}`);
//         },
//         onCancel: async () => {
...
//         amount: chargeAmount,
//         email,
//         reference: intentData.reference, // ← use Paystack's own unique reference
//         channels: method === "bank" ? ["bank_transfer"] : ["card", "bank", "ussd", "qr", "mobile_money", "bank_transfer"],
//         metadata: { order_id: order.id, order_code: order.code, payment_method: method },
//         onSuccess: () => {
//           // Directly wipe localStorage — no stale closure risk
//           // LocalCartContext uses "localCart:v1" as its storage key
//           try {
